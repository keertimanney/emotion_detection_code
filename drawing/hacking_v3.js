...
function draw() {
    ...
    let int = random(0.5,1);
    let index= parseInt(Math.ceil(random() * 3));
    let rd = [255,0,0]; let gr=[0,255,0]; let bl=[0,0,255];
    let inp = [rd,gr,bl];
    var col= random(inp);

    let int2 = random(0.5,1);
    var col2 = random(inp);

    let int3 = random(0.5,1);
    var col3 = random(inp);

    let limit1 = ceil(5*int);
    let limit2 = ceil(5*int2);
    let limit3 = ceil(5*int3);
   ...
   if (current_time - last_updated_time > 2000.0) {
        ...
        starting_point_1 = iterate(starting_point_1, col, int, limit1);
        starting_point_2 = iterate(starting_point_2, col2, int2, limit2);
        starting_point_3 = iterate(starting_point_3, col3, int3, limit3);
       ...
       if (updated) {
            starting_point_1 = iterate(starting_point_1, col, int, -1);
            starting_point_2 = iterate(starting_point_2, col2, int2, -1);
            starting_point_3 = iterate(starting_point_3, col3, int3, -1);
...
function iterate(starting_point_1, col, int, limit) {
    ...
    for (i=0; i<l; i++) {
      let intensity = starting_point_1.intHist[i] * ((i + 1) / l);
        ...
        let new_color = [color[0] * intensity * int, color[1] * intensity * int, color[2] * intensity * int];
        ...
        let posx = starting_point_1.a + s*r-ceil(random(1*(r-n)));
        let posy = starting_point_1.b + s*r-ceil(random(1*(r-n)));
...
//         posx = starting_point_1.a + 2*s*r+ceil(random(1*(r-n)));
//         posy = starting_point_1.b + 2*s*r-ceil(random(1*(r-n)));
//         console.log("Whats the point x and point y", posx, posy);
//         // rect(posx, posy, movementx, movementy);
//         starting_point_1 = fill_colors(starting_point_1, posx, posy, movementx, movementy, new_color, rectx, recty);

//         posx = starting_point_1.a + s*r-ceil(random(1*(r-n)));
//         posy = starting_point_1.b + s*r-ceil(random(1*(r-n)));
//         console.log("Whats the point x and point y", posx, posy);
//         // rect(posx, posy, movementx, movementy);
//         starting_point_1 = fill_colors(starting_point_1, posx, posy, movementx, movementy, new_color, rectx, recty);
